...
import "./Affiliate_Reward_Modal.css";

interface Affiliate_Reward_Modal_Props {
  show: boolean;
  onClose: () => void;
  onButtonClick: () => void;
}

export default function Affiliate_Reward_Modal({ show, onClose, onButtonClick }: Affiliate_Reward_Modal_Props) {
  const [isVisible, setIsVisible] = useState(false);
  const modalRef = useRef<HTMLDivElement | null>(null);
  const t = useTranslations("AffiliateRewardModal");
  ...
  useEffect(() => {
    if (show) {
      ...
      modalElement.id = 'affiliate-reward-modal-portal';
      ...
      modalContent.innerHTML = `
        <div style="text-align: center; color: white;">
          ...
          <div style="margin-bottom: 1.5rem; text-align: center;">
            <img src="/assets/images/logo_slogan.png" alt="Oxygen" style="height: 4rem; width: auto; margin-bottom: 0; display: inline-block;" />
          </div>
          
          <!-- Recompensa destacada -->
          <div style="background: linear-gradient(135deg, rgba(0, 202, 166, 0.2) 0%, rgba(0, 106, 106, 0.2) 100%); border: 2px solid rgba(0, 202, 166, 0.5); border-radius: 0.75rem; padding: 1rem; margin-bottom: 1.5rem;">
            <div style="font-size: 1.75rem; font-weight: 800; color: rgba(0, 202, 166, 1); margin-bottom: 0.25rem;">
              +5 OM
            </div>
            <div style="font-size: 0.875rem; font-weight: 600; color: rgba(255, 255, 255, 0.9);">
              ${t("reward")}
            </div>
          </div>
          ...
          <!-- Botón -->
          <button id="affiliate-reward-modal-button" style="
