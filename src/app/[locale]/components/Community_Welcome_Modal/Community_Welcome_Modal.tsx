...
import "./Community_Welcome_Modal.css";

interface Community_Welcome_Modal_Props {
  show: boolean;
  onClose: () => void;
  onButtonClick: () => void;
}

export default function Community_Welcome_Modal({ show, onClose, onButtonClick }: Community_Welcome_Modal_Props) {
  const [isVisible, setIsVisible] = useState(false);
  const modalRef = useRef<HTMLDivElement | null>(null);
  const t = useTranslations("CommunityWelcomeModal");
  ...
  useEffect(() => {
    if (show) {
      ...
      modalElement.id = 'community-welcome-modal-portal';
      ...
      modalContent.innerHTML = `
        <div style="text-align: center; color: white;">
          ...
          <div style="margin-bottom: 1.5rem;">
            <h2 style="font-size: 1.5rem; font-weight: 700; color: white; margin-bottom: 0.5rem; line-height: 1.3;">
              ${t("title")}
            </h2>
          </div>
          ...
          <!-- Botón -->
          <button id="community-welcome-modal-button" style="
      ...
      const button = modalContent.querySelector('#community-welcome-modal-button') as HTMLElement;
      if (button) {
