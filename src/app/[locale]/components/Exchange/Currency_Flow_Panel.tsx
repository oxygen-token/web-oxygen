...
export default function Currency_Flow_Panel() {
  const [selectedTimeframe, setSelectedTimeframe] = useState('1w');
  const [price] = useState('0000.00');
      ...
      <div className="flex items-start justify-between mb-2 sm:mb-3">
        <div className="flex items-start gap-2 sm:gap-3">
          <div className="flex items-center gap-1 sm:gap-2">
            ...
            <div className="w-6 h-6 sm:w-8 sm:h-8 bg-green-500 rounded-full flex items-center justify-center -ml-1 sm:-ml-2">
              <span className="text-white text-xs font-bold">T</span>
            </div>
          </div>
          <div>
            <div className="text-white font-semibold text-base sm:text-lg">OM / USDT</div>
            <div className="flex items-baseline gap-2 mt-0.5 sm:mt-1">
              <span className="text-white text-xl sm:text-2xl font-bold">{price}</span>
              <span className="text-[#5EEAD4] text-xs sm:text-sm">{change}</span>
            </div>
          </div>
        </div>
